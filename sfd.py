...
for it in range(1000):
    if not first == True:
        ...
        sfd_click(ahk,(offset+500,60))
    first = False
    val=cmpCurrentAct(loc["page2"],"./res/page2.png")
    if (val<0.2):
        print("fight page2....")

        sfd_click(ahk,act["go"])
        time.sleep(0.5)

        print("check Challenge ... ")
        chk=cmpCurrentAct(loc["challenge"],"./res/challenge.png")
        if (chk<0.5):
            sfd_click(ahk,loc["challenge"])
            time.sleep(0.5)


        a1=cmpCurrentAct(loc["fight"],"./res/fight.png")
        if (a1<0.2):
            sfd_click(ahk,act["fight"])
            for i in range(30):
                time.sleep(2)
                b1=cmpCurrentAct(loc["result"],"./res/result.png")
                print("{} Check win -> {}".format(i,b1))
                if (b1<0.5):
                    time.sleep(2)
                    sfd_click(ahk,(offset+500,60))
                    time.sleep(2)
